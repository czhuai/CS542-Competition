...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # check whether this is a numeric question
        ...
        # get background passages
        if 'background' in example and len(example['background']) > 0 and self.n_context is not None:
            # f = self.title_prefix + " {} " + self.passage_prefix + " {}"
            f = self.passage_prefix + " {}"
            if len(example['background']) < self.n_context: # if we don't have enough articles
                add_on = self.n_context - len(example['background'])
                example['background'].extend([example['background'][0]] * add_on)
            contexts = example['background'][:self.n_context]
            # passages = [f.format(c['title'], c['text']) for c in contexts]
            passages = example['background']
    ...
    def sort_data(self):
        if self.n_context is None or not 'score' in self.data[0]['background'][0]:
            return
        for ex in self.data:
            ex['background'].sort(key=lambda x: float(x['score']), reverse=True)
...
class Collator(object):
    ...
    def __call__(self, batch):
        ...
        #     if example['passages'] is None:
        #         return [example['question']]
        #     return [example['question'] + " " + example['passages']]
